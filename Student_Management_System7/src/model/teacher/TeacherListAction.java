 ...
 *
 */
public class TeacherListAction extends ActionSupport implements HttpSessionListener,SessionAware,ServletRequestAware,ServletResponseAware {
	...
	public String view(){
		...
		String sessionteacherId=(String)session.getAttribute("sessionteacherId");
		try{
			Connection con= DataBaseConnection.getConnection();
			String sql=null,searchCondition="",condition="",sortCondition="",sortConditionFiled="teacherId";
			//For filter Process
			if(filterId==0)
			...
			if(this.searchText != null && this.searchText.length()>0){
				searchCondition=" And teacherId LIKE '%"+searchText.trim()+"%' OR teacherName LIKE '%"+searchText.trim()+"%' OR mailId LIKE '%"+searchText.trim()+"%'";
			}
			//order Condition
			if(this.sortField==2)
			{sortConditionFiled="teacherName";}
			...
			String countIdSql = "SELECT COUNT(id) as count FROM tbl_teacher Where".concat(condition).concat(searchCondition);
			...
			session.setAttribute("sessionuser","Teacher");
			sql="SELECT*FROM tbl_teacher Where".concat(condition).concat(searchCondition).concat(sortCondition)+" LIMIT "+(pageid-1)+" , "+recordsPerPage;
	...
	String status ="UseFail";
		try {
			Connection con= DataBaseConnection.getConnection();
			PreparedStatement useUpdate = con.prepareStatement("Update tbl_teacher set delflag=0 where teacherId = ?");
	...
	String status ="NotUseFail";
		try {
			Connection con= DataBaseConnection.getConnection();
			PreparedStatement notUseUpdate = con.prepareStatement("Update tbl_teacher set delflag=1 where teacherId = ?");
	...
	public void setServletRequest(HttpServletRequest request) {
		TeacherListAction.request=request;
