...
import model.course.CourseListDAO;
...
public class CourseListAction extends ActionSupport implements
...
ServletResponseAware {
	...
	String sortConditionFiled = "courseId";
	...
	public String courseview() throws Exception {
		...
		try {
			...
			if (this.searchText != null && this.searchText.length() > 0) {
				searchCondition = " And courseId LIKE '%" + searchText.trim()
						+ "%' OR courseName LIKE '%" + searchText.trim() + "%'";
			}
			// order Condition
			if (this.sortField == 2) {
				sortConditionFiled = "courseName";
			}
			...
			String countIdSql = "SELECT COUNT(id) as count FROM tbl_course Where"
			...
			session.setAttribute("sessionuser", "Course");
			
			status = CourseListDAO.courseview(this);
		} catch (Exception e) {
	...
	public String courselistUse() {
		String status = "UseSuccess";
		status = CourseListDAO.courselistUse(this);
		return status;
	}

	public String coursenotlistUse() {
		String status = "NotUseSuccess";
		status = CourseListDAO.coursenotlistUse(this);
		return status;
	}
