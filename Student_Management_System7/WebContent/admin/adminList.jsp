...
<%
	String name = (String) session.getAttribute("sessionadminId");
...
<script>
	function fnUse(adminId) {
		$('#hdnuserid').val(adminId);
		$('#viewForm').attr('action', 'NotUse');
		$('#viewForm').submit();
	}
	function fnNotUse(adminId) {
		$('#hdnuserid').val(adminId);
		$('#viewForm').attr('action', 'Use');
		$('#viewForm').submit();
	}
	function fnSearch() {
...
<body>
	...
	<fieldset class="mt1per">
		<div style="width: 100%; display: block;">
			<div style="width: 2%; display: inline-block;">
				<img alt="User" src="image/signup.png" width="42">
			</div>
			<div style="width: 96%; display: inline-block;">
				<h1 class=" h1header">Admin List</h1>
			</div>
		</div>
	</fieldset>
	...
	<form name="viewForm" id="viewForm" class="model-content"
		action="ListRedirect" method="post" accept-charset="UTF-8">
		...
		<div class="maindiv">
			<div class="regbtndiv">
				<a href="registerbtn" class="regbtn">Register</a>
			</div>
			<div class="searchbox">
		...
		<div class="maindiv">
			...
			<div class="selectbox">
				<s:select cssStyle="width:100%;padding-top:8px;"
					list="#{'1':'Admin Id','2':'Admin Name'}" id="sortField"
			...
			<s:iterator value="list" status="listStatus">
				<tr>
					...
					<td><a href="javascript:;"
						onclick="fnSingleView('${adminId}')"><s:property
								value="adminId" /></a></td>
					...
					<td><s:set name="delflag" value="delflag" /> <s:if
							...
							<a class="green" href="javascript:;"
								onclick="fnUse('${adminId}')"><%="Use"%></a>
						</s:if> <s:else>
							<a class="red" href="javascript:;"
								onclick="fnNotUse('${adminId}')"><%="NotUse"%></a>
						</s:else></td>
				</tr>
			</s:iterator>
		</s:if>
